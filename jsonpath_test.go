...
func TestExtractRows(t *testing.T) {
	cases := []struct {
		...
		{name: "missing path segment", body: `{"data":{}}`, rowsPath: "data.rows", wantErr: true},
		{name: "path through non-object", body: `{"data":1}`, rowsPath: "data.rows", wantErr: true},
		{name: "row is not an object", body: `[1,2]`, wantErr: true},
		{name: "rows value is a scalar", body: `{"data":1}`, rowsPath: "data", wantErr: true},
